...
<div class="action">
		...
		<%-- <input type="hidden" name="pageNum"  value="${page.pageNum}"/> --%>
				...
				<c:forEach items="${page.data}" var="email" varStatus="i">
					...
					<td align="left" width="10%"><a href="javascript:void(0)" onclick="toRecover(${email.id})">还原</a>&nbsp|&nbsp
					<a href="javascript:void(0)" onclick="todelete(${email.id})">删除</a></td>
				</tr>
				</c:forEach>
				</c:if>
				<tr>
...
<script type="text/javascript">
		...
		function updateIsdelete(id){
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/emailAction/deleteEmail.action',
				dataType:'text',
				data:{'model.id':id},
		...
		//回复邮件
		function recoverdelete(id){
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/emailAction/recoverIsdelete.action',
				dataType:'text',
				data:{'model.id':id},
				success:function(data){
					if(data=="true"){
